...
.setTitle('𐌞anity • #2021 - TAG Alındı.')
...
log.send(new Discord.MessageEmbed()
.setTitle('𐌞anity • #2021 - TAG Çıkarıldı.')
.setColor('#f1c335')
...
if(mesaj.includes('𐌞anity • #2021')) message.react('🤫');
})
...
client.on('message', async message => {
  ...
  message.delete();
  message.channel.send(new Discord.MessageEmbed().setTitle('Küfür engellenmiştir.').setDescription(`${message.author} ${uyarımesaj}`));
  }
  })
...
client.on('message', async msg => {
 ...
 const kalpgif = client.emojis.cache.get('775351638734405653');
    msg.reply(`**Aleyküm Selam, Günaydın ** `)
}});
client.on('message', async msg => {
  if (msg.content.toLowerCase() === 'sa günaydın') {
 const kalpgif = client.emojis.cache.get('775351638734405653');
